"""FastAPI entry point for the Tax & Assessment Agent.
...
from agents.tax_assistance_agent.graph import build_graph  # noqa: E402

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger("tax_assistance_agent")
...
async def lifespan(app: FastAPI):
    logger.info("Building LangGraph runnable (LLM + Pinecone MCP + Tax DB MCP + Payment MCP + A2A tool)...")
    app.state.graph = await build_graph()
    logger.info("Tax & Assessment Agent ready.")
    yield


app = FastAPI(title="Tax & Assessment Agent", lifespan=lifespan)
...
if __name__ == "__main__":
    uvicorn.run("agents.tax_assistance_agent.main:app", host="0.0.0.0", port=PORT)
