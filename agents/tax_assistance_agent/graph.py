...
Dynamically discovers tools from THREE MCP servers (pinecone-kb, tax-db-mcp,
payment-mcp), pins the Pinecone tool's namespace to this department's
KB_NAMESPACE, and binds an agent-to-agent tool that forwards out-of-scope
questions to a running Citizen Inquiry Agent instance over HTTP.
"""
import logging
import os
import string
...
TAX_DB_MCP_SERVER_PATH = _resolve_mcp_server("mcp_servers/tax_db_mcp/server.py")
PAYMENT_MCP_SERVER_PATH = _resolve_mcp_server("mcp_servers/payment_mcp/server.py")

# When running standalone (no monorepo data/ directory available), point both
# tax-db-mcp and payment-mcp (which share this DB) at a writable path next to
# this agent instead — their server.py modules auto-create/seed on first run.
_TAX_DB_PATH = (
    REPO_ROOT / "data" / "tax_revenue.db"
    if (REPO_ROOT / "data").exists()
    else AGENT_DIR / "data" / "tax_revenue.db"
)
    ...
    client = MultiServerMCPClient(
        {
            ...
            "tax-db-mcp": {
                "transport": "stdio",
                "command": sys.executable,
                "args": [str(TAX_DB_MCP_SERVER_PATH)],
                "env": {**os.environ, "TAX_DB_PATH": str(_TAX_DB_PATH)},
            },
            "payment-mcp": {
                "transport": "stdio",
                "command": sys.executable,
                "args": [str(PAYMENT_MCP_SERVER_PATH)],
                "env": {**os.environ, "TAX_DB_PATH": str(_TAX_DB_PATH)},
            },
        }
    )
