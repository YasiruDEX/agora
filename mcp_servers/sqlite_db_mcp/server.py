...
The database file is configurable via SQLITE_DB_PATH so this server also works
when bundled standalone alongside a single agent (e.g. a flattened deployment
that doesn't have the rest of the monorepo, and therefore doesn't have a real
REPO_ROOT to compute a default path from). Falls back to the standard
monorepo-relative path (data/social_services.db) when unset.
"""
import json
import os
import sqlite3
...
def _resolve_db_path() -> Path:
    raw_path = os.environ.get("SQLITE_DB_PATH", str(REPO_ROOT / "data" / "social_services.db"))
    path = Path(raw_path)
...
mcp = FastMCP("sqlite-db", host=MCP_HOST, port=MCP_PORT)
...
if __name__ == "__main__":
    ...
    if transport != "stdio":
        print(f"[sqlite-db-mcp] serving over {transport} at {MCP_HOST}:{MCP_PORT}", file=sys.stderr)
